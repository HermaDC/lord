...
void print_error_at(const char *error_str, const char *line_str, Token actual_token) {
...
ASTNode *parse_primary(Parser *p) {
    ...
    if (match(p, TOKEN_NUMBER)) {
        ...
        if (tok->lexeme)
            return make_number(p->arena, atof(tok->lexeme));
    }
    if (match(p, TOKEN_TRUE_LITERAL)) {
        return make_true(p->arena);
    }
    if (match(p, TOKEN_FALSE_LITERAL)) {
        return make_false(p->arena);
    }
    ...
    if (match(p, TOKEN_LEFT_PAREN)) {
        ...
        if (!match(p, TOKEN_RIGHT_PAREN)) {
            parser_error(p, "SyntaxError: expected ')'");
        }
        return left;
    }
