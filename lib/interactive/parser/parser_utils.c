...
Arena *arena_create(size_t chunk_size) {
    if (chunk_size == 0)
        chunk_size = ARENA_DEFAULT_CHUNK;
...
ASTNode *make_if(Arena *arena, ASTNode *condition, ASTNode *if_branch, ASTNode *else_branch) {
    if (!arena || !condition || !if_branch)
        return NULL;
...
void push_to_block(Arena *arena, ASTNode *parent, ASTNode *child) {
    ...
    if (parent->block.count >= parent->block.capacity) {
        ...
        memcpy(new_children, parent->block.children, parent->block.count * sizeof(ASTNode *));
