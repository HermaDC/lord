...
char* hints(const char *buf, int *color, int *bold) { //use with the param lime print and in red <id>
    for(size_t i = 0; commands[i].name != NULL; i++) {
...
int interactive_main_loop(void) {
    ...
    if (home) {
        ...
        fprintf(stderr, "Could not get HOME environment variable, history will not be saved\n");
    }
