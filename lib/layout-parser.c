...
static int push_token(LayoutToken **tokens, size_t *count, size_t *capacity,
                      TokenType type, int value, size_t column) {
    if(*count >= *capacity) {
        *capacity *= 2;
        LayoutToken *tmp = realloc(*tokens, *capacity * sizeof(*tmp));
        if(!tmp) return 0;
        *tokens = tmp;
    }
...
LayoutToken *tokenize_layout(const char *str, size_t *num_tokens,
    ...
    *num_tokens = 0;
    ...
    LayoutToken *tokens = NULL;
    tokens = malloc(buffer_size * sizeof(*tokens));
    ...
    while(str[i] != '\0') {
        ...
        if(isdigit(c)) {
            ...
            if(!push_token(&tokens, num_tokens, &buffer_size, NUMBER, value, start))
                goto error;
        } else if(c == '(') {
        ...
        // If is SW add a token with type SW and value 0
            ...
            *error_code = TOKENIZE_OK;
            ...
            fprintf(stderr,
                    RED "Syntax Error: " RESET "Unexpected character '%c'\n"
                        "Column: %zu\n",
                    c, i);
...
void print_layout_config_tokens(LayoutToken *tokens_arr, size_t count) {
...
int check_syntax_layout_config(LayoutToken *tokens_arr, char *original_str, size_t count,
                               TokenizeError *error_code) {
    if(count == 0) {
        *error_code = TOKENIZE_OK;
        return 0;
    ...
    if(tokens_arr[0].type != NUMBER) {
        fprintf(stderr,
                RED "Syntax Error:" RESET " The config file must begin with a number\n"
                    "Column: %zu\n",
                tokens_arr[0].column);
    ...
    if(tokens_arr[count - 1].type != NUMBER) {
        fprintf(stderr,
                RED "Syntax Error:" RESET " The config file must end with a number\n"
                    "Column: %zu\n",
    ...
    for(size_t i = 0; i < count; i++) {
        if(tokens_arr[i].type == SW) {
            if(i + 1 >= count || tokens_arr[i + 1].type != OPEN) {
                fprintf(stderr,
                        RED "Syntax Error:" RESET
                            " SW must be followed by an opening parenthesis\n"
                            "Column: %zu\n",
                        tokens_arr[i].column);
            ...
            if(tokens_arr[i].value <= 0) {
                fprintf(stderr,
                        RED "Syntax Error:" RESET
                            " Number of tracks must be greater than 0\n"
                            "Column: %zu\n",
                        tokens_arr[i].column);
                *error_code = TOKENIZE_MISSING_NUM;
                print_error_at(original_str, tokens_arr[i].column);
            ...
            if(i + 1 >= count || tokens_arr[i + 1].type != NUMBER) {
                fprintf(stderr,
                        RED "Syntax Error:" RESET
                            " Parenthesis must be followed by a valid number\n"
                            "Column: %zu\n",
                        tokens_arr[i].column);
                *error_code = TOKENIZE_MISSING_NUM;
                print_error_at(original_str, tokens_arr[i].column);
            ...
            if(balance < 0) {
                fprintf(stderr,
                        RED "Syntax Error: " RESET "Unmatched closing parenthesis\n"
                            "Column: %zu\n",
                        tokens_arr[i].column);
                *error_code = TOKENIZE_UNMATCHED_PARENTHESES;
                print_error_at(original_str, tokens_arr[i].column);
            ...
            if(i == 0 || tokens_arr[i - 1].type != NUMBER) {
                fprintf(stderr,
                        RED "Syntax Error: " RESET
                            "Missing number before closing parenthesis\n"
                            "Column: %zu\n",
                        tokens_arr[i].column);
                *error_code = TOKENIZE_MISSING_NUM;
