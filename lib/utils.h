...
// Creates a switch and inserts it in system with the params, if id is 0 will generate one
// automatically
ErrorCode insert_switch(System *system, int id, Sensor *sensor, int track_next,
                        int track_prev, int branch);

// Creates a num_tracks straight line of tracks in system, in index stores the index of
// the first track of the line
...
// Loads the system layout from a file, returns an array of systems, and stores in
// out_count the number of systems loaded
